...
const LessonList = () => {
  ...
  const fetchLessons = async () => {
    try {
      ...
      handleError(error, 'Failed to fetch lessons');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      handleError(error, 'Failed to delete lesson');
    }
  };
                  ...
                  lessons.map((lesson) => (
                    <tr key={lesson.id} className="hover:bg-gray-50">
                      <td className="px-6 py-4">
                        <div className="font-medium">{lesson.subject?.subjectName || lesson.subject?.name || 'N/A'}</div>
                      </td>
                      <td className="px-6 py-4 text-sm">{lesson.class?.name || 'N/A'}</td>
                      <td className="px-6 py-4 text-sm">{lesson.teacher?.name || lesson.teacher?.user?.name || 'N/A'}</td>
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-2">
                          <Calendar size={16} className="text-gray-400" />
                          <span className="text-sm">{lesson.dayOfWeek || 'N/A'}</span>
                        </div>
                      </td>
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-2">
